...
class OrderControllerCore extends ParentOrderController
{
    ...
    public function initContent()
    {
        ...
        switch ((int)$this->step) {
            ...
            case OrderController::STEP_ADDRESSES:
                ...
                $db = Db::getInstance();
                $sql = new DbQueryCore();
                $customer = new CustomerCore(Context::getContext()->customer->id);
                ...
                if (count($addresses)>1) {
                    foreach ($addresses as $addr) {
                        if ($addr['type'] == 'invoice') {
                            $cart->id_address_invoice = $addr['id_address'];
                        } elseif ($addr['type'] == 'delivery') {
                            $cart->id_address_delivery = $addr['id_address'];
                        }
                    }
                } else {
                    $cart->id_address_delivery = $addresses[0]['id_address'];
                    $cart->id_address_invoice = $addresses[0]['id_address'];
                }
                ...
                if (Tools::getValue('multi-shipping') == 1) {
                    ...
                    $this->context->smarty->assign(
                        array(
                            ...
                            'delivery_address' => $this->formatAddress($cart->id_address_delivery),
                            'invoice_address' => $this->formatAddress($cart->id_address_invoice),
                        )
                    );
            ...
            case OrderController::STEP_DELIVERY:
                ...
                Context::getContext()->cart->id_address_delivery = Tools::getValue('id_address_delivery', 0);
                Context::getContext()->cart->id_address_invoice = Tools::getValue('id_address_invoice', 0);
                $this->autoStep();
                $this->_assignCarrier();
    ...
    private function formatAddress($id_address)
    {
        ...
        $output = array(
            ...
            'phone_mobile' => $address->phone_mobile?$address->phone_mobile:'',
            'vat_number' => $address->vat_number?sprintf('Partita IVA: %s', $address->vat_number):'',
            'dni' => $address->dni?sprintf('Codice Fiscale: %s', $address->dni):'',
        );
        return $output;
    }
