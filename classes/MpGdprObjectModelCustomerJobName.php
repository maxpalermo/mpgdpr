...
class MpGdprObjectModelCustomerJobName extends ObjectModelCore
{
    /** @var MpGdpr $module Module */
    private $module;
    /** @var date $id_lang language reference */
    public $id_lang;
    /** @var string $name job name */
    public $name;
    /** @var string $tablename Table Name */
    private $tablename = 'mp_customer_job_name';
    /** @var array Definitions */
    public static $definition = array(
        'table' => 'mp_customer_job_name',
        'primary' => 'id_mp_customer_job_name',
        'multilang' => false,
        'fields' => array(
            'id_lang' => array(
                'type' => self::TYPE_INT,
    ...
    public function installTable()
    {
        $sql[] = "CREATE TABLE IF NOT EXISTS `"._DB_PREFIX_.$this->tablename."` (
              `id_mp_customer_job_name` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
    ...
    public function displayForm()
    {
        ...
        $form->submit_action = 'submitAddJobName';
    ...
    private function getFieldsValue()
    {
        return array(
            ...
            'input_text_job_name_id' => Tools::getValue('input_text_job_name_id', 0),
            'input_text_job_name_name' => Tools::getValue('input_text_job_name_name', ''),
        );
    }
        ...
        $fields_form = array(
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        'type' => 'text',
                        'label' => $this->module->l('Job name id', get_class($this)),
                        'name' => 'input_text_job_name_id',
                        'class' => 'fixed-width-sm text-right',
                    ),
                    array(
                        'type' => 'text',
                        'label' => $this->module->l('Job name', get_class($this)),
                        'name' => 'input_text_job_name_name',
                        'class' => 'fixed-width-xxl',
                    ),
                ),
                'buttons' => array(
                    'submitSaveJobName' => array(
                        'title' => $this->module->l('Save Job Name', get_class($this)),
                        'icon' => 'process-icon-save',
                        'href' => 'javascript:$(\'#mp_customer_job_name_form\').submit();',
                    ),
                ),
            )
        );
    ...
    public function displayList()
    {
        ...
        $list->identifier = 'id_mp_customer_job_name';
        $list->no_link = true;
        ...
        $list->table = 'mp_customer_job_name';
    ...
    private function getHelperListFields()
    {
        $fields_list = array(
            'id_mp_customer_job_name' => array(
                ...
                'align' => 'right',
                'type' => 'text',
                'filter_key' => 'a\!id_mp_customer_job_name',
                'order_by' => true,
                'search' => true,
                'hint' => $this->module->l('Job name code'),
            ),
            'name' => array(
                'title' => $this->module->l('Job name', get_class($this)),
                'width' => 'auto',
    ...
    private function getHelperListContent()
    {
        ...
        $sql->select('*')
            ->from($this->tablename)
            ->orderBy('id_mp_customer_job_name');
    ...
    public static function getListByArea($id_area, $id_lang = 0)
    {
        if ($id_lang==0) {
            $id_lang = (int)Context::getContext()->language->id;
        }
        $db = Db::getInstance();
        $sql = new DbQueryCore();
        $sql->select('jn.id_mp_customer_job_name')
            ->select('jn.name')
            ->from('mp_customer_job_name', 'jn')
            ->innerJoin('mp_customer_jobs', 'j', 'j.id_mp_customer_job_name=jn.id_mp_customer_job_name')
            ->where('jn.id_lang='.(int)$id_lang)
            ->where('j.id_mp_customer_job_area='.(int)$id_area)
            ->orderBy('jn.name');
        $result = $db->executeS($sql);
        return $result;
    }
